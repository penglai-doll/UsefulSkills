...
python3 skill-creator/scripts/quick_validate.py linux-loader
```

最后一条命令从仓库根目录运行（使用本地的 skill-creator 脚本）。如果 `quick_validate.py` 不在本地或因缺少 PyYAML 失败，先按本地依赖问题处理，再用手动 frontmatter 检查兜底。
